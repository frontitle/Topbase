GO ?= go
VERSION ?= $(shell cat VERSION 2>/dev/null || echo 0.2.1-dev)
...
.PHONY: help version fmt fmt-check vet js-check test build check run backup
...
build:
	...
	$(GO) build -trimpath -ldflags "$(LDFLAGS)" -o bin/topbase-cli ./cmd/topbase-cli
	$(GO) build -trimpath -ldflags "$(LDFLAGS)" -o bin/topbase-mcp ./cmd/topbase-mcp
